...
private class AdbSocket(private val opener: (host: String, port: Int) -> AdbStream) : Socket() {
    ...
    override fun getInputStream(): InputStream {
        val activeStream = stream ?: throw SocketException("Socket is not connected")
        return activeStream.source.inputStream()
    }

    override fun getOutputStream(): OutputStream {
        val activeStream = stream ?: throw SocketException("Socket is not connected")
        return object : FilterOutputStream(activeStream.sink.outputStream()) {
